...
import { AliadoService, AliadoWrap, Locales, MediaResponse, PersonaRelacionadaWrap } from 'aliados';
...
export class AliadoComponent extends CrudImpl implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.activeRouter.params.subscribe(params => {
      if (params['nroaliado']){
        ...
        this.form.reset()
        this.service.getAliadoWrap(this.libEnvService.getConfig().ciaopr.ciaopr,params['nroaliado']).subscribe(
          result=>{
            if (result){
            ...
            this.snack.msgSnackBar(error.error,'OK',undefined,'error')
            this.showSpinner = false
          }
        )
  ...
  // ngOnDestroy(): void {
      // this.resumenEditor.destroy()
      // this.desc3Editor.destroy()
  // }
  ...
  override modificar(): void {
      if (this.form.status === 'VALID'){
        super.modificar()
        this.showSpinner=true
        let aliado:AliadoWrap = this.form.getRawValue()
        ...
        const anoInicioActividades = `${this.form.value.persona.fechanacimiento}/01/01`
        // aliado.persona.fechanacimiento = moment(this.form.value.persona.fechanacimiento).format('YYYY/MM/DD')
        aliado.persona.fechanacimiento = moment(anoInicioActividades).format('YYYY/MM/DD')
        ...
        if (!this.tipNipJuridico(aliado.persona.tipnip)){
          aliado.persona.siglaspersjuridica = aliado.persona.nombreprimero + " " + aliado.persona.apellidoprimero
          aliado.persona.nombrepersjuridica = aliado.persona.nombreprimero + " " + aliado.persona.apellidoprimero
        }

        // VERIFICACIÓN DE QUE SEA UN OBJETO DE NGX-EDITOR PARA CONVERTIRSE EN TIPO HTML
        // if (this.form.get('aliado_presentacion')?.value != null && 
        //   Object.prototype.hasOwnProperty.call(this.form.value.aliado_presentacion,'type')){
        //   aliado.aliado_presentacion = toHTML(this.form.get('aliado_presentacion')?.value)
        // }
        // if (this.form.get('aliado_descripcion1')?.value != null && 
        //     Object.prototype.hasOwnProperty.call(this.form.value.aliado_descripcion1,'type')){
        //   aliado.aliado_descripcion1 = toHTML(this.form.get('aliado_descripcion1')?.value)
        // } 
        ...
        this.service.createAliado(aliado.ciaopr,aliado).subscribe(
          result=>{
            if (result) {
              ...
              this.snack.msgSnackBar('Aliado Modificado exitósamente','OK',undefined,'success')
              this.showSpinner=false
  ...
  buscarPerRelacionada():void {
    ...
    catalogoPersonaJuridica():void{
      this.dialog.open(CatalogoPersonaComponent,
        {data: this.dataDialogo('Búsqueda de Personas',undefined,undefined,undefined,25,undefined)}
      ).afterClosed().subscribe(result=>{
        if(result){
  ...
  setAliado(aliado:AliadoWrap):void{
    ...
    if (typeof aliado.persona.fechanacimiento != undefined){
      ...
      aliado.persona.fechanacimiento = moment(aliado.persona.fechanacimiento).format('YYYY') || ''
    }
  ...
  appendTags(e:FocusEvent):void{
    ...
    let tags:string[] = []
    let tagsExistentes = this.form.get('aliado_descripcion2')?.value
  ...
  removeDuplicateTags(e:FocusEvent){
    ...
    this.form.patchValue({
      aliado_descripcion2:tagsSinRepetir.join(', '),
    })
  }
  ...
  changeNip(e:MatSelectChange):void{
    if (e.value == 'TJ'){
      this.persona_juridica.get('codnip')?.setValue('0')
      this.persona_juridica.get('codnip')?.disable()
    } else {
      this.persona_juridica.get('codnip')?.setValue('')
      this.persona_juridica.get('codnip')?.enable()
    }
  }

  tipNipJuridico(tipnip:string):boolean{

    const tipNip = this.tablasApoyo.tipNip.find(tn=>tn.tipnipcod === tipnip)

    if (tipNip?.personatipocod === 'J'){
      return true;
    }else{
      this.form.patchValue({
        persona:{
          siglaspersjuridica : this.form.value.persona.nombreprimero + " " + this.form.value.persona.apellidoprimero,
          nombrepersjuridica : this.form.value.persona.nombreprimero + " " + this.form.value.persona.apellidoprimero,
        }
      })
      return false;
    }
  ...
  uploadFile(formControlName:string,fileName:string,modalTitle:string):void{
    ...
    const filePath = `/xityclub/${this.form.value.nroaliado}/`
