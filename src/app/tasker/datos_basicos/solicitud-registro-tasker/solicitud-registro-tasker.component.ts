...
import { AliadoService, CategoriaTasker, Locales, TaskerService, TaskerView } from 'aliados';
...
import { CatalogoTaskerTemporalComponent } from 'src/app/shared/borrame/catalogo-tasker-temporal/catalogo-tasker-temporal.component';
import { CrudImpl } from 'vcore';

@Component({
  selector: 'app-solicitud-registro-tasker',
  templateUrl: './solicitud-registro-tasker.component.html',
  styleUrls: ['./solicitud-registro-tasker.component.css']
})
export class SolicitudRegistroTaskerComponent extends CrudImpl implements OnInit{
  ...
  override consultar(): void {
    ...
    this.locales.clear()
    this.avatar=''
    this.dialog.open(CatalogoTaskerTemporalComponent,{data:this.dataDialogo("Búsqueda de Tasker",undefined,undefined,undefined,25)})
  ...
  override incluir(): void {
    if (this.form.status == 'VALID'){
      ...
      this.service.upsertRegTasker(tasker.ciaopr,tasker).subscribe(
        result=>{
          if(result){
            this.form.reset()
      ...
      this.service.upsertRegTasker(tasker.ciaopr,tasker).subscribe(
        result=>{
          if(result){
            ...
            this.dialog.open(CatalogoGenericoComponent,{height:'250px', width:'350px', data:this.dataDialogo('Aviso',`El registro de ${this.form.value.persona.nombreprimero} ${this.form.value.persona.apellidoprimero} fué actualizado, para convertirlo en tasker, debe hacerlo utilizando el botón CREAR TASKER`,'Ok','Cerrar',undefined)}).afterClosed().subscribe(
              ()=>{
                  return
                }
              )
  ...
  creaTasker(){
    ...
    this.dialog.open(CatalogoGenericoComponent,{height:'250px', width:'350px', data:this.dataDialogo('Confirmar Tasker',`Está seguro de que desea convertir en Tasker a ${this.form.value.persona.nombreprimero} ${this.form.value.persona.apellidoprimero}`,'Registrar','Regresar',undefined)}).afterClosed().subscribe(
      result=>{
        if(result){
          this.guardaTasker()
        }else {
      ...
      this.service.creaTasker(tasker.ciaopr,tasker).subscribe(
        result=>{
          if(result){
            ...
            this.service.setNroTaskerRegistro(result.ciaopr,result).subscribe(result=>{console.log(result)})
            ...
            this.dialog.open(CatalogoGenericoComponent,{height:'250px', width:'350px', data:this.dataDialogo('Aviso',`El registro de ${this.form.value.persona.nombreprimero} ${this.form.value.persona.apellidoprimero} fué creado, para convertirlo en tasker, debe hacerlo utilizando el botón CREAR TASKER`,'Ok','Cerrar',undefined)}).afterClosed().subscribe(
              ()=>{
                  return
                }
              )
